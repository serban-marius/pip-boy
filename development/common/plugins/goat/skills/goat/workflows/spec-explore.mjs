...
// args: { ticket: string, hints?: string[] } — should arrive as a real object.
// Tolerate a JSON-encoded string (a common caller mistake that otherwise leaves ticket undefined).
let parsedArgs = args
if (typeof parsedArgs === 'string') {
  try { parsedArgs = JSON.parse(parsedArgs) } catch { parsedArgs = {} }
}
const ticket = parsedArgs?.ticket ?? '(no ticket text provided)'
const hints = Array.isArray(parsedArgs?.hints) ? parsedArgs.hints : []

// Abort outright: with no ticket the explorers would run blind and the synthesized spec would
// look confident while targeting the wrong problem. Don't spend agents on a run we know is bad.
if (ticket === '(no ticket text provided)') {
  log('⚠️ spec-explore received NO ticket text (args.ticket is empty). Aborting — pass `args` as a JSON OBJECT with a real `ticket` (including any attachment/screenshot content) and re-run.')
  return { spec: null, facts: '', error: 'args.ticket is empty — workflow aborted before exploring' }
}
...
// Optional subset via args.angles (e.g. ['entrypoints','precedent','tests']) — the skill passes
// 3 angles for small tickets/bugfixes, all 6 for features. Unknown keys ignored; empty or no match -> all.
const requested = Array.isArray(parsedArgs?.angles) ? parsedArgs.angles : []
const selected = requested.length ? ANGLES.filter((a) => requested.includes(a.key)) : []
const angles = selected.length ? selected : ANGLES
if (angles.length < ANGLES.length) log(`Angles: ${angles.map((a) => a.key).join(', ')} (${ANGLES.length - angles.length} skipped for ticket size)`)
...
phase('Explore')
// Explorers run on sonnet/low: grep-and-cite is mechanical, but the facts must be accurate
// (the synthesizer trusts them blindly) — sonnet holds that bar, haiku doesn't.
const explored = await parallel(
  angles.map((a) => () =>
    agent(
      ...
      { label: `explore:${a.key}`, phase: 'Explore', schema: FINDINGS_SCHEMA, agentType: 'Explore', model: 'sonnet', effort: 'low' },
    ),
  ),
)
