...
// args: { spec: object|string, ticketKey?: string, hints?: string[], facts?: string }
...
// Grounded findings from spec-explore — already paid for once; architects verify against them
// instead of each re-exploring the whole repo from scratch.
const facts = typeof args?.facts === 'string' ? args.facts.trim() : ''

const context = `Ticket: ${ticketKey}

Approved spec:
${specText}${hints.length ? `\n\nHints to focus on:\n- ${hints.join('\n- ')}` : ''}${facts ? `\n\nGrounded findings from the spec exploration (evidence-backed, file:line cited):\n${facts}` : ''}

${facts
  ? 'START from the grounded findings above — they already map the relevant code. Grep/read the repo ONLY to verify a finding you build on or to cover what they miss; do not re-explore from scratch.'
...
phase('Panel')
// Panel on sonnet: the architects GENERATE options; the expensive judgment happens once, in the
// session-model synthesizer that reconciles them. Normal effort — design still needs thinking.
const designs = await parallel(
  LENSES.map((lens) => () =>
    agent(
      `${lens.prompt}\n\n${context}`,
      { label: `architect:${lens.key}`, phase: 'Panel', schema: DESIGN_SCHEMA, model: 'sonnet' },
