...
const LENSES = [
  ...
  // conventions is the one mechanical lens (compare against sibling files) — cheap model is enough.
  // The bug-hunting lenses stay on the session model: a missed real bug has no vote to recover it.
  { key: 'conventions', model: 'sonnet', prompt: 'Hunt for broken project conventions, naming, dead code, leftover debug, and design debt that a maintainer would have to fix later. Read sibling files to learn the conventions first.' },
...
// Scale the panel to the diff: a small change rarely hides security/convention issues, and low
// findings never gate. ponytail: line-count heuristic, revisit if a small diff ever ships a vuln.
const SMALL_DIFF_LINES = 150
const isSmall = typeof diff === 'string' && diff.split('\n').length < SMALL_DIFF_LINES
const lenses = isSmall ? LENSES.filter((l) => ['correctness', 'edge-cases', 'test-quality'].includes(l.key)) : LENSES
if (isSmall) log(`Small diff (<${SMALL_DIFF_LINES} lines): ${lenses.length} lenses instead of ${LENSES.length}`)
...
const reviews = await parallel(
  lenses.map((lens) => () =>
    agent(
      ...
      { label: `review:${lens.key}`, phase: 'Review', schema: FINDINGS_SCHEMA, ...(lens.model ? { model: lens.model } : {}) },
    ),
  ),
)
...
// Haiku has held up fine here in real runs — the majority vote absorbs an occasional bad refute.
