#!/usr/bin/env python3
"""Per-feature lock + watch state + pacing for the geiger post-deploy loop.
Keyed by a PR/branch slug so several features can be watched independently.
State dir: ~/.claude/state/geiger/<key>/ (state.json, baseline.json, .lock)."""
import argparse, json, os, shutil, sys
from datetime import datetime, timezone
from pathlib import Path

STATE_ROOT = Path.home() / ".claude" / "state" / "geiger"
PACING = [60, 270, 900]
BUMP_AT = [3, 8]
...
def _lock_acquire(key):
    ...
    if lock_path.exists():
        ...
        if age is not None and age < LOCK_FRESH_WINDOW_S:
            die(f"geiger already watching '{key}' (last activity {int(age)}s ago). "
                f"Use `/geiger --stop {key}` first.", code=2)
